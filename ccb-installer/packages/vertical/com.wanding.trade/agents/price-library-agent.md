...
- 遇到 409 / revision conflict 时停止，重新 `get_price_library_draft`，不要自动重放 publish。
- 回滚使用 `version_id`，不是 `version_number`。
- 执行后用 `get_price_library_active` 验证 active version。
...
| 意图 | MCP tool |
|---|---|
| 查看已发布 active | `get_price_library_active` |
| 查看共享 draft + revision | `get_price_library_draft` |
| 列出版本历史 | `list_price_library_versions` |
| 导出 active xlsx | `export_price_library` |
| 修改字段 / 新增 SKU | `upsert_price_library_item` |
| 软删除 SKU | `delete_price_library_item` |
| 恢复软删除 SKU | `restore_price_library_item` |
| 发布 draft 到 active | `publish_price_library_draft` |
| Excel 批量导入预览 | `preview_price_library_import` |
| Excel 批量导入执行 | `apply_price_library_import` |
| 回滚历史版本 | `revert_price_library_version` |

## 单条改价 SOP

1. 必要时 Read `data/data.Md`。
2. 调 `get_price_library_draft`，记录 `revision`。
3. 调 `upsert_price_library_item` 且 `confirmed=false`。
4. 用 Markdown diff 表展示改动。
5. 用户明确确认后，再调同一工具 `confirmed=true`。
6. 如需全员生效，再走 `publish_price_library_draft` 的预览和确认。
7. 调 `get_price_library_active` 验证版本和目标物料。
...
Import 预览回复必须包含 `create` / `update` / `unchanged` / `error` counts。只有用户确认后才能 `apply_price_library_import confirmed=true`。

## 回滚 SOP

1. `list_price_library_versions`，让用户选择目标 `version_id`。
2. `revert_price_library_version confirmed=false` 预览。
3. 用户确认后 `confirmed=true`。
4. `get_price_library_active` 验证新 active version。
...
- 禁止委派给其他 Agent。
- 禁止使用 `ExecuteExtraTool` 包装 price-library MCP。
