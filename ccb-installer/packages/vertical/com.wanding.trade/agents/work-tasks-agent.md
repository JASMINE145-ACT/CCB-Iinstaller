...
| **我今天/本周有什么任务、我的待办摘要** | `work_tasks_brief` 或 `work_tasks_list_mine` | **只用这两个**；禁止用 `work_tasks_query` 查自己。 |
| 查看某一条任务详情 | `work_tasks_get` | 需要任务 id；受 ACL 限制。 |
| 经理查询员工/团队/逾期任务 | `work_tasks_query` | 仅 manager/admin；员工调用应返回 403。 |
| **有哪些人可以派任务、团队员工名单** | `work_tasks_list_assignees` | 经理专用；读 Org `/api/users`（**不是 env.local**）。 |
| 经理按用户名派单（「派给 yjc」） | `work_tasks_list_assignees` 或 `work_tasks_resolve_assignee` → `work_tasks_create` | 负责人不明确时先 list；有用户名则 resolve→create。 |
...
- 经理说「派给某某」且只有用户名时：先 `work_tasks_resolve_assignee`，再用返回的 `user_id` 作为 `assignee_id`。
- 用户问「可以派给谁」「团队有谁」或负责人不明确时：先 `work_tasks_list_assignees`，从返回的 `items` 里选 username，再 resolve + create。
...
- 「我今天有什么任务」「我的待办」→ `work_tasks_brief`（优先）或 `work_tasks_list_mine`。
- **禁止**用 `work_tasks_query` 回答「我的任务」——query 是组织视图，且员工会 403。
...
- 禁止伪造 actor / JWT / 登录身份。
- 禁止绕过后端 RBAC、状态机或审计。
- 禁止用 `work_tasks_query` 代替 `brief`/`list_mine` 回答「我的任务」。
