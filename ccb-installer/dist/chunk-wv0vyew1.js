...
import {
  QueryEngine
} from "./chunk-j69d40x9.js";
import"./chunk-jz79mq04.js";
import"./chunk-cnsj3vq2.js";
import"./chunk-fk90c2eg.js";
...
function resolvePermissionMode(defaultMode) {
    ...
    while (!abortSignal.aborted) {
      const nextResult = await Promise.race([
        ...
        new Promise((resolve2) => {
          ...
          const handler = () => resolve2({ done: true, value: undefined });
...
var IS_ROOT2 = typeof process.geteuid === "function" ? process.geteuid() === 0 : typeof process.getuid === "function" ? process.getuid() === 0 : false;
var ALLOW_BYPASS2 = !IS_ROOT2 || !!process.env.IS_SANDBOX;
function createAcpCanUseTool(conn, sessionId, getCurrentMode, clientCapabilities, cwd) {
  return async (tool, input, _context, _assistantMessage, toolUseID, _forceDecision) => {
...
async function handleExitPlanMode(conn, sessionId, toolUseID, input, supportsTerminalOutput, cwd) {
  const options = [
  ...
  if (response.outcome.outcome === "selected" && "optionId" in response.outcome && response.outcome.optionId !== undefined) {
    ...
    if (selectedOption === "default" || selectedOption === "acceptEdits" || selectedOption === "auto" || selectedOption === "bypassPermissions") {
      await conn.sessionUpdate({
        sessionId,
        update: {
...
class AcpAgent {
  ...
  async prompt(params) {
    ...
    if (session.promptRunning) {
      ...
      const cancelled = await new Promise((resolve2) => {
        session.pendingMessages.set(promptUuid, { resolve: resolve2, order });
      });
      if (cancelled) {
  ...
  async createSession(params, opts = {}) {
    ...
    const permissionMode = resolvePermissionMode(this.getSetting("permissions.defaultMode"));
    const canUseTool = createAcpCanUseTool(this.conn, sessionId, () => this.sessions.get(sessionId)?.modes.currentModeId ?? "default", this.clientCapabilities, cwd);
    const appState = {
      ...getDefaultAppState(),
