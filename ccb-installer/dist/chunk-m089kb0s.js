...
async function selectEngine() {
  if (process.platform === "win32") {
    const { DetachedEngine: DetachedEngine2 } = await import("./chunk-jpbkdg47.js");
    return new DetachedEngine2;
  }
  const { TmuxEngine } = await import("./chunk-hxb3js2g.js");
  const tmux = new TmuxEngine;
  if (await tmux.available()) {
    return tmux;
  }
  const { DetachedEngine } = await import("./chunk-jpbkdg47.js");
