...
function defaultClaudeConfigDir() {
    if (process.platform === "win32" && process.env.LOCALAPPDATA) {
        return path.join(process.env.LOCALAPPDATA, "CCB-Wanding", ".claude");
    }
    return path.join(os.homedir(), ".claude");
}
export const CLAUDE_CONFIG_DIR = process.env.CLAUDE_CONFIG_DIR ?? defaultClaudeConfigDir();
process.env.CLAUDE_CONFIG_DIR = CLAUDE_CONFIG_DIR;
const CCB_WANDING_SDK_LOG = process.env.CCB_WANDING_SDK_LOG ??
    path.join(os.tmpdir(), "aionui-ccb-wanding-sdk.log");
function appendCcbWandingSdkLog(message) {
    try {
        fs.mkdirSync(path.dirname(CCB_WANDING_SDK_LOG), { recursive: true });
        fs.appendFileSync(CCB_WANDING_SDK_LOG, `[${new Date().toISOString()}] ${message}\n`);
    }
    catch {
    }
}
const CCB_WANDING_INIT_TIMEOUT_MS = 5000;
/** Default wait for SDK query.next. Hot path unchanged (returns as soon as SDK emits). */
const CCB_WANDING_QUERY_NEXT_TIMEOUT_DEFAULT_MS = 120_000;
const CCB_WANDING_QUERY_NEXT_TIMEOUT_MIN_MS = 30_000;
const CCB_WANDING_QUERY_NEXT_TIMEOUT_MAX_MS = 300_000;
function resolveQueryNextTimeoutMs() {
    const raw = process.env.CCB_WANDING_QUERY_NEXT_TIMEOUT_MS;
    if (raw === undefined || raw === "") {
        return CCB_WANDING_QUERY_NEXT_TIMEOUT_DEFAULT_MS;
    }
    const parsed = Number.parseInt(String(raw), 10);
    if (!Number.isFinite(parsed)) {
        return CCB_WANDING_QUERY_NEXT_TIMEOUT_DEFAULT_MS;
    }
    return Math.min(CCB_WANDING_QUERY_NEXT_TIMEOUT_MAX_MS, Math.max(CCB_WANDING_QUERY_NEXT_TIMEOUT_MIN_MS, parsed));
}
const CCB_WANDING_QUERY_NEXT_TIMEOUT_MS = resolveQueryNextTimeoutMs();
const CCB_WANDING_MCP_ALL = ["quotation", "accurate"];
// Both modes use project+local only — exclude "user" to avoid MCP double-start Bun panic (Round 3.1).
// Wanding MCP (quotation/accurate) is injected via options.mcpServers in native mode.
const CCB_WANDING_SDK_ADAPT_SETTING_SOURCES = ["project", "local"];
function ccbWandingNativeModeEnabled() {
    return process.env.CCB_WANDING_ACP_NATIVE !== "0";
}
function ccbWandingResolveInstallDir() {
    for (const installDir of ccbWandingInstallDirs()) {
...
function ccbWandingFindGitBash(installDir) {
    const candidates = [
        installDir ? path.join(installDir, "vendor", "git", "bin", "bash.exe") : undefined,
        "D:\\Git\\bin\\bash.exe",
...
function ccbWandingMcpConfigPath() {
    const installDir = ccbWandingResolveInstallDir();
    if (!installDir) {
        return undefined;
    }
    const configPath = path.join(installDir, "ccb-mcp.json");
    return fs.existsSync(configPath) ? configPath : undefined;
}
/** AionUI conversation dirs often have broken .claude symlinks; use install root instead. */
function ccbWandingResolveSessionCwd(cwd) {
    const installDir = ccbWandingResolveInstallDir();
    if (!cwd || typeof cwd !== "string") {
        return installDir ?? cwd;
    }
    const normalized = cwd.replace(/\\/g, "/").toLowerCase();
    if (normalized.includes(".aionui-web/conversations/") ||
        normalized.includes("claude-temp-")) {
        if (installDir) {
            appendCcbWandingSdkLog(`cwd override ${cwd} -> ${installDir}`);
            return installDir;
        }
    }
    return cwd;
}
function ccbWandingNativeSpawnEnv(settingsEnv) {
    const installDir = ccbWandingResolveInstallDir();
    const pathParts = [];
    if (installDir) {
        pathParts.push(path.join(installDir, "vendor", "bun"), path.join(installDir, "vendor", "ripgrep"), path.join(installDir, "vendor", "git", "bin"), path.join(installDir, "vendor", "git", "mingw64", "bin"), path.join(installDir, "vendor", "git", "usr", "bin"));
    }
    const existingPath = process.env.PATH ?? "";
    const gitBash = ccbWandingFindGitBash(installDir);
    return {
        ...process.env,
        LANG: "zh_CN.UTF-8",
        LC_ALL: "zh_CN.UTF-8",
        ...
        CLAUDE_CODE_DISABLE_EARLY_INPUT: "1",
        CLAUDE_CODE_DISABLE_FAST_MODE: process.env.CLAUDE_CODE_DISABLE_FAST_MODE ?? "1",
        CCB_WANDING_SKIP_GROVE: process.env.CCB_WANDING_SKIP_GROVE ?? "1",
        CLAUDE_CODE_DISABLE_GIT_INSTRUCTIONS: process.env.CLAUDE_CODE_DISABLE_GIT_INSTRUCTIONS ?? "1",
        ...
        CLAUDE_CONFIG_DIR: process.env.CLAUDE_CONFIG_DIR ?? CLAUDE_CONFIG_DIR,
        PATH: pathParts.length > 0 ? `${pathParts.join(path.delimiter)}${path.delimiter}${existingPath}` : existingPath,
        ...(gitBash ? { CLAUDE_CODE_GIT_BASH_PATH: gitBash } : {}),
        ...settingsEnv,
    };
}
// Native mode always loads full Wanding MCP set. Legacy sdk-adapt defaults to accurate-only.
function ccbWandingAcpMcpNames(nativeLaunch = false) {
    if (nativeLaunch || process.env.CCB_WANDING_ACP_INCLUDE_QUOTATION === "1") {
        return [...CCB_WANDING_MCP_ALL];
    }
    appendCcbWandingSdkLog("mcp policy accurate-only (quotation disabled; set CCB_WANDING_ACP_INCLUDE_QUOTATION=1 to re-enable)");
    return ["accurate"];
}
function resolveCcbWandingEffectiveModel(settingsEnv, settings) {
    const candidate = settingsEnv.ANTHROPIC_MODEL ||
        settings.model ||
        settingsEnv.ANTHROPIC_DEFAULT_SONNET_MODEL ||
        settingsEnv.ANTHROPIC_DEFAULT_OPUS_MODEL ||
        "minimax-m3";
    return candidate === "default" ? "minimax-m3" : candidate;
}
function mergeCcbWandingMcpServers(existing, required) {
    const result = { ...existing };
    for (const [name, config] of Object.entries(required)) {
        if (!result[name]) {
            result[name] = config;
            appendCcbWandingSdkLog(`mcp inject ${name}`);
        }
        else {
            appendCcbWandingSdkLog(`mcp skip existing ${name}`);
        }
    }
    return result;
}
function logCcbWandingSpawnEnv(label, env) {
    appendCcbWandingSdkLog(`${label} HOME=${process.env.HOME ?? ""} USERPROFILE=${process.env.USERPROFILE ?? ""}`);
    appendCcbWandingSdkLog(`${label} CLAUDE_CONFIG_DIR=${env.CLAUDE_CONFIG_DIR ?? process.env.CLAUDE_CONFIG_DIR ?? ""}`);
    appendCcbWandingSdkLog(`${label} ANTHROPIC_BASE_URL=${env.ANTHROPIC_BASE_URL ?? "missing"}`);
    appendCcbWandingSdkLog(`${label} ANTHROPIC_API_KEY=${env.ANTHROPIC_API_KEY ? "set" : "missing"}`);
    appendCcbWandingSdkLog(`${label} ANTHROPIC_AUTH_TOKEN=${env.ANTHROPIC_AUTH_TOKEN ? "set" : "missing"}`);
    appendCcbWandingSdkLog(`${label} ANTHROPIC_MODEL=${env.ANTHROPIC_MODEL ?? "missing"}`);
}
async function queryNextWithTimeout(queryInstance, sessionId, ms = CCB_WANDING_QUERY_NEXT_TIMEOUT_MS) {
    let timeoutId;
    const timeoutPromise = new Promise((_, reject) => {
        timeoutId = setTimeout(() => {
            reject(new Error(`query.next timeout after ${ms}ms`));
        }, ms);
    });
    try {
        const result = await Promise.race([queryInstance.next(), timeoutPromise]);
...
function ccbWandingFallbackInitializationResult(settingsEnv, settings) {
    const model = settingsEnv.ANTHROPIC_MODEL ||
        settings.model ||
        settingsEnv.ANTHROPIC_DEFAULT_SONNET_MODEL ||
        settingsEnv.ANTHROPIC_DEFAULT_OPUS_MODEL ||
        "minimax-m3";
        ...
        models: [
            {
                value: "default",
                displayName: "Default",
                description: "CCB-Wanding default",
            },
            {
                value: model,
                displayName: model,
                description: "CCB-Wanding",
            },
        ],
    };
}
function resolveCcbWandingPermissionMode(settings, logger) {
    const mode = resolvePermissionMode(settings.permissions?.defaultMode, logger);
    if (mode === "default" && ALLOW_BYPASS) {
        return "bypassPermissions";
    }
    return mode;
}
function ccbWandingMcpServersFromSettings(settings, nativeLaunch = false) {
    const configured = settings.mcpServers || {};
    const selected = {};
    for (const name of ccbWandingAcpMcpNames(nativeLaunch)) {
        const server = configured[name];
        if (!server) {
            appendCcbWandingSdkLog(`mcp missing ${name}`);
            continue;
        }
...
export async function claudeCliPath() {
    ...
    if (process.platform === "win32") {
        for (const installDir of ccbWandingInstallDirs()) {
            const cli = path.join(installDir, "dist", "cli.js");
            const bun = path.join(installDir, "vendor", "bun", "bun.exe");
            if (fs.existsSync(cli) && fs.existsSync(bun)) {
                return cli;
            }
        }
    }
...
function ccbWandingInstallDirs() {
    return [
        "D:\\CCB-Wanding",
        path.join(process.env.LOCALAPPDATA ?? "", "Programs", "CCB-Wanding"),
    ];
}
function ccbWandingBunExecutable() {
    if (process.platform !== "win32" || process.env.CLAUDE_CODE_EXECUTABLE) {
        return undefined;
    }
    for (const installDir of ccbWandingInstallDirs()) {
        const bun = path.join(installDir, "vendor", "bun", "bun.exe");
...
export class ClaudeAcpAgent {
    ...
    async prompt(params) {
        ...
        const promptPreview = params.prompt
            ...
            .join(" ")
            .slice(0, 120);
        appendCcbWandingSdkLog(`prompt start sessionId=${params.sessionId} text=${JSON.stringify(promptPreview)}`);
        appendCcbWandingSdkLog(`prompt context sessionId=${params.sessionId} model=${session.ccbEffectiveModel ?? session.models.currentModelId} cwd=${session.cwd} config=${process.env.CLAUDE_CONFIG_DIR} base_url=${session.ccbBaseUrl ?? "missing"} mcp=${(session.ccbMcpNames ?? []).join(",")}`);
        session.cancelled = false;
        session.promptRetryCount = 0; // reset per-prompt retry counter (max 1 silent retry on query.next timeout)
        ...
        // retry_prompt loops at most twice: once normally, once on silent timeout retry
        retry_prompt: while (true) {
        session.promptRunning = true; // re-assert after finally may have cleared it on retry
        let handedOff = false;
        let errored = false;
        let stopReason = "end_turn";
        try {
            while (true) {
                appendCcbWandingSdkLog(`prompt next wait sessionId=${params.sessionId}`);
                const { value: message, done } = await queryNextWithTimeout(session.query, params.sessionId);
                appendCcbWandingSdkLog(`prompt next sessionId=${params.sessionId} done=${done} type=${message?.type ?? "none"} subtype=${message?.subtype ?? "none"}`);
                if (done || !message) {
            ...
            const isTimeout = message.includes("query.next timeout");
            if (isTimeout) {
                appendCcbWandingSdkLog(`prompt next timeout sessionId=${params.sessionId} model=${session.ccbEffectiveModel ?? session.models.currentModelId} base_url=${session.ccbBaseUrl ?? "missing"} mcp=${(session.ccbMcpNames ?? []).join(",")}`);
            }
            ...
            if (message.includes("ProcessTransport") ||
                ...
                const bunCrash = message.includes("code 3") || message.includes("exited with code 3");
                throw RequestError.internalError(undefined, bunCrash
                    ? "CCB-Wanding 子进程崩溃（Bun exit code 3）。请重启 AionUI 后重试；若仍失败，检查 MCP 是否重复启动。"
        ...
        break retry_prompt; // normal exit; only continue above causes another iteration
        } // end retry_prompt while(true)
    }
    async cancel(params) {
        const session = this.sessions[params.sessionId];
        if (!session) {
    ...
    async unstable_setSessionModel(params) {
        ...
        appendCcbWandingSdkLog(`setModel local-only sessionId=${params.sessionId} model=${modelId}`);
    ...
    async setSessionConfigOption(params) {
        ...
        if (params.configId === "mode") {
            ...
            appendCcbWandingSdkLog(`setSessionConfigOption model local-only sessionId=${params.sessionId} model=${resolvedValue}`);
        }
    ...
    async applySessionMode(sessionId, modeId) {
        ...
        appendCcbWandingSdkLog(`setMode local-only sessionId=${sessionId} mode=${modeId}`);
    }
    ...
    async sendAvailableCommandsUpdate(sessionId) {
        ...
        if (!session)
            return;
        // CCB-Wanding can hang on query.supportedCommands(); it also races with
        // the first session/prompt on the same SDK query iterator.
        appendCcbWandingSdkLog(`supportedCommands skipped sessionId=${sessionId}`);
        await this.client.sessionUpdate({
            sessionId,
            update: {
    ...
    async applyConfigOptionValue(sessionId, session, configId, value) {
        if (configId === "mode") {
            ...
            if (!newAvailableModes.some((m) => m.id === previousModeId)) {
                ...
                appendCcbWandingSdkLog(`setPermissionMode default skipped after model switch sessionId=${sessionId}`);
                modeDowngraded = true;
            }
            else {
            ...
            if (newEffort !== currentEffort) {
                appendCcbWandingSdkLog(`applyFlagSettings effort skipped sessionId=${sessionId} effort=${toSdkEffortLevel(newEffort)}`);
            }
            ...
            session.configOptions = session.configOptions.map((o) => o.id === configId && typeof o.currentValue === "string" ? { ...o, currentValue: value } : o);
            if (configId === "effort") {
                appendCcbWandingSdkLog(`applyFlagSettings effort local-only sessionId=${sessionId} effort=${toSdkEffortLevel(value)}`);
            }
        }
    }
    ...
    async createSession(params, creationOpts = {}) {
        ...
        const sessionCwd = ccbWandingResolveSessionCwd(params.cwd);
        const input = new Pushable();
        const settingsManager = new SettingsManager(sessionCwd, {
            logger: this.logger,
        });
        ...
        const effectiveSettings = settingsManager.getSettings();
        const settingsEnv = effectiveSettings.env || {};
        const ccbWandingAuthToken = settingsEnv.ANTHROPIC_AUTH_TOKEN || process.env.ANTHROPIC_AUTH_TOKEN;
        const ccbWandingApiKey = settingsEnv.ANTHROPIC_API_KEY || process.env.ANTHROPIC_API_KEY || ccbWandingAuthToken;
        const permissionMode = resolveCcbWandingPermissionMode(effectiveSettings, this.logger);
        // Extract options from _meta if provided
        const sessionMeta = params._meta;
        const userProvidedOptions = sessionMeta?.claudeCode?.options;
        const effectiveModel = resolveCcbWandingEffectiveModel(settingsEnv, effectiveSettings);
        const nativeLaunch = ccbWandingNativeModeEnabled();
        const aionuiMcpServersRaw = { ...mcpServers, ...(userProvidedOptions?.mcpServers || {}) };
        const aionuiMcpServers = nativeLaunch
            ? Object.fromEntries(Object.entries(aionuiMcpServersRaw).filter(([name]) => !CCB_WANDING_MCP_ALL.includes(name)))
            : aionuiMcpServersRaw;
        // Native mode: inject quotation/accurate via options (stream-json does not
        // reliably start user-source MCP). Exclude "user" from settingSources to
        // avoid double-start Bun panic. Do NOT pass --mcp-config extraArgs.
        const wandingMcp = ccbWandingMcpServersFromSettings(effectiveSettings, nativeLaunch);
        const mergedMcpServers = mergeCcbWandingMcpServers(aionuiMcpServers, wandingMcp);
        const settingSources = CCB_WANDING_SDK_ADAPT_SETTING_SOURCES;
        const baseSpawnEnv = nativeLaunch ? ccbWandingNativeSpawnEnv(settingsEnv) : { ...process.env, ...settingsEnv };
        if (nativeLaunch) {
            const mcpConfig = ccbWandingMcpConfigPath();
            appendCcbWandingSdkLog(`launch mode=native install=${ccbWandingResolveInstallDir() ?? "missing"} mcp-config=${mcpConfig ?? "none"} wanding-mcp=options-inject(${Object.keys(wandingMcp).join(",") || "none"})`);
        }
        else {
            appendCcbWandingSdkLog("launch mode=sdk-adapt (CCB_WANDING_ACP_NATIVE=0)");
        }
        ...
        const options = {
            ...
            env: {
                ...baseSpawnEnv,
                ...(ccbWandingApiKey ? { ANTHROPIC_API_KEY: ccbWandingApiKey } : {}),
                ...(ccbWandingAuthToken ? { ANTHROPIC_AUTH_TOKEN: ccbWandingAuthToken } : {}),
            ...
            executable: userProvidedOptions?.executable ?? ccbWandingBunExecutable(),
            mcpServers: mergedMcpServers,
            debug: true,
            debugFile: CCB_WANDING_SDK_LOG,
            stderr: (data) => appendCcbWandingSdkLog(`stderr ${data}`),
        ...
        logCcbWandingSpawnEnv(`spawn sessionId=${sessionId}`, options.env);
        appendCcbWandingSdkLog(`query start sessionId=${sessionId} mode=${nativeLaunch ? "native" : "sdk-adapt"} model=${effectiveModel} cwd=${sessionCwd} config=${process.env.CLAUDE_CONFIG_DIR} settingSources=${settingSources.join(",")} mcp=${Object.keys(mergedMcpServers).join(",")} cli=${options.pathToClaudeCodeExecutable} executable=${options.executable} permissionMode=${permissionMode}`);
        const q = query({
            prompt: input,
            options,
        });
        ...
        try {
            appendCcbWandingSdkLog(`initializationResult wait sessionId=${sessionId}`);
            const initializationPromise = q.initializationResult();
            initializationResult = await Promise.race([
                initializationPromise,
                new Promise((resolve) => setTimeout(() => resolve(ccbWandingFallbackInitializationResult(settingsEnv, effectiveSettings)), CCB_WANDING_INIT_TIMEOUT_MS)),
            ]);
            initializationPromise
                .then(() => appendCcbWandingSdkLog(`initializationResult late-ok sessionId=${sessionId}`))
                .catch((error) => appendCcbWandingSdkLog(`initializationResult late-error sessionId=${sessionId} ${error instanceof Error ? error.stack || error.message : String(error)}`));
            appendCcbWandingSdkLog(`initializationResult ready sessionId=${sessionId} models=${initializationResult.models?.map((m) => m.value).join(",")}`);
        }
        catch (error) {
            appendCcbWandingSdkLog(`initializationResult error sessionId=${sessionId} ${error instanceof Error ? error.stack || error.message : String(error)}`);
            ...
            if (error instanceof Error && error.message.includes("exited with code 3")) {
                throw RequestError.internalError(undefined, "CCB-Wanding 子进程在 session 初始化时崩溃（Bun exit code 3）。常见原因是 MCP 重复启动，请重启 AionUI 后重试。");
            }
            throw error;
        }
        ...
        let models = await getAvailableModels(q, allowedModels, initializationResult.models, settingsManager, this.logger);
        if (models.currentModelId === "default" || !models.currentModelId) {
            models = { ...models, currentModelId: effectiveModel };
            appendCcbWandingSdkLog(`model override sessionId=${sessionId} effective=${effectiveModel}`);
        }
        ...
        if (!availableModes.some((m) => m.id === effectiveMode)) {
            ...
            appendCcbWandingSdkLog(`setPermissionMode default local-only sessionId=${sessionId}`);
        }
        const modes = {
        ...
        if (initialEffort &&
            ...
            appendCcbWandingSdkLog(`applyFlagSettings initial effort local-only sessionId=${sessionId} effort=${initialEffort.currentValue}`);
        }
        this.sessions[sessionId] = {
            ...
            cancelled: false,
            cwd: sessionCwd,
            sessionFingerprint: computeSessionFingerprint({ ...params, cwd: sessionCwd }),
            settingsManager,
            accumulatedUsage: {
                inputTokens: 0,
                outputTokens: 0,
            ...
            taskState,
            ccbEffectiveModel: effectiveModel,
            ccbBaseUrl: settingsEnv.ANTHROPIC_BASE_URL || process.env.ANTHROPIC_BASE_URL,
            ccbMcpNames: Object.keys(mergedMcpServers),
        };
        return {
            sessionId,
            models,
...
async function getAvailableModels(query, models, sdkModels, settingsManager, logger) {
    ...
    if (!skipSetModel) {
        appendCcbWandingSdkLog(`getAvailableModels setModel local-only model=${currentModel.value}`);
    }
    return {
