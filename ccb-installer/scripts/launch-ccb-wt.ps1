...
# ASCII-only: no Chinese strings in this file.

$scriptDir  = Split-Path -Parent $MyInvocation.MyCommand.Path
$installDir = Split-Path -Parent $scriptDir
$cmdFile    = Join-Path $installDir "ccb.cmd"

# Mark as already relaunched so ccb.cmd skips its own WT detection.
...
if ($wtExe -and (Test-Path $wtExe)) {
    ...
    # Launch WT directly. CCB_WT_RELAUNCHED=1 is inherited so ccb.cmd
