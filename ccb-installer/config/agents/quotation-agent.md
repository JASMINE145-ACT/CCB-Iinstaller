...
| 何时 | 读什么 | 路径 |
|------|--------|------|
| 多候选 / 品类语义 / 用户纠偏 | 业务知识库 shadow | `D:\CCB-Wanding\vendor\wanding\data\wanding_business_knowledge.md` |
...
流程：**先 match → Read 知识库一次（本轮所有 match 返回后）→ 同一条回复输出推荐价**。
...
```json
{
  "fill_items": [ { "row": 8, "code": "8020020755", "inquiry_name": "直接50", "quote_name": "...", "unit_price": 1519, "qty": 1, "specification": "dn50" } ],
...
填表后：`read_data_from_excel` 抽检；备注 O / 单格修正 → `write_data_to_excel`。**禁止**整表重写 F–N；**禁止** COM `excel-mcp.exe`。
...
- **查价**：表格含产品、规格、英文/印尼名称、编码、客户等级、单价、备注。
