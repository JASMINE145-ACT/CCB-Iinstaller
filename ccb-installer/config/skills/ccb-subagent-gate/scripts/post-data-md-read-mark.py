#!/usr/bin/env python3
"""PostToolUse: mark session after Read(data.Md) for PreToolUse price-library gate."""
...
from parse_transcript_data_md_gate import (  # noqa: E402
    _payload_has_data_md,
    mark_session_data_md_read,
)


def _tool_input_is_data_md_read(tool_name: str, tool_input: object) -> bool:
    ...
    if file_path and _payload_has_data_md(file_path):
        return True
    return _payload_has_data_md(json.dumps(tool_input, ensure_ascii=False))
...
def main() -> int:
    ...
    if isinstance(tool_response, dict) and tool_response.get("is_error") is True:
        return 0
    if not _tool_input_is_data_md_read(tool_name, tool_input):
        return 0

    session_id = str(hook_input.get("session_id") or "").strip()
    mark_session_data_md_read(session_id)
    return 0
