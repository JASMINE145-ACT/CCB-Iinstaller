#!/usr/bin/env bash
# quotation-mcp.sh — MCP evidence check (warn or block per mode)
set -euo pipefail
...
tool_pattern='mcp__quotation__match_quotation(_batch)?'
if has_successful_mcp_call "$transcript_path" "$tool_pattern"; then
  exit 0
fi

reason="claimed quotation output but no successful mcp__quotation__* in transcript"
if [[ "$mode" == "block" ]]; then
