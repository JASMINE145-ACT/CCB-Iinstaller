...
- `quotation-agent`: MCP evidence check **off**; knowledge Read gate **off** (2026-07-19 — selection is `select_quotation_candidates` API-first; agent Read only on `unable_to_select` fallback). PreToolUse `pre-match-knowledge-gate.py` **removed** from L1. PostToolUse `post-knowledge-read-mark.py` still marks session when fallback Read happens; PostToolUse `post-match-knowledge-nudge.py` nudges **hybrid + select** after match (hybrid on every successful match; select lines when multi-candidate); `post-quotation-relay-nudge.py` after `select_quotation_candidates` → `status:ok` (nudge locked code + 货源段 into next assistant text; `modes.json` `:relay-guard: off` = not a Stop gate — PostToolUse still fires when L1 wires it); `post-price-tiers-nudge.py` when `get_product_price_tiers` returns tiers.
...
- `wande-orchestrator`: base `off`; **`wande-orchestrator:outcome-relay` → `block`** (2026-07-16) — parent Stop checks Agent artifact delivery into parent bubble (`WANd.ORCH.OUTCOME_RELAY.001`).
## Agent Guidance
