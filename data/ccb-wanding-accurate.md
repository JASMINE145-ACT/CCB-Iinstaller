# CCB-Wanding Accurate Analysis Rules

> **加载要求（2026-06-16 起）**：本文件仍是 Accurate SOP 的**维护源**；运行时已内联到 `agents/accurate-agent.md` 正文，agent **不要**每轮 Read 本文件。修改本文件后须手动同步 agent 正文。不要从 index 或记忆推断参数。
...
```
Step 1  accurate_search_records   → 确认 vendor/customer ID
Step 2  accurate_summarize_records × N   → 每个主体各调一次
...
只有当没有聚合需求、只是要查看原始列表时，才使用 `accurate_fetch_by_date`。
