...
function resolveDistDir() {
    ...
    const looksLikeDist = (dir) => {
        return (fs_1.default.existsSync(path_1.default.join(dir, 'run_host.sh')) ||
            fs_1.default.existsSync(path_1.default.join(dir, 'run_host.bat')) ||
            fs_1.default.existsSync(path_1.default.join(dir, 'node_path.txt')));
    };
...
async function collectDoctorReport(options) {
    ...
    const packageName = typeof pkg.name === 'string' ? pkg.name : constant_1.PACKAGE_NAME;
    ...
    const detectedBrowsers = (0, browser_config_1.detectInstalledBrowsers)();
    ...
    if (missingHostFiles.length > 0) {
        ...
        nextSteps.push(`Reinstall: npm install -g ${constant_1.PACKAGE_NAME}`);
    }
    else {
        checks.push({
            id: 'host.files',
    ...
    if (noBrowserDetected) {
        checks.push({
            ...
            status: 'warn',
            message: 'No supported browser (Chrome/Chromium) detected in this environment. ' +
    ...
    const report = {
        ...
        environment: {
            ...
            nativeHost: { hostName: constant_1.HOST_NAME },
        },
        fixes,
        checks,
